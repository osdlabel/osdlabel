...
- `ViewerControlId` is now `'exposure' | 'contrast'`, and the drag parameters for both live in the new shared `VIEWER_CONTROL_SPECS` registry so Solid and React behave identically. The controls take different axes: exposure drags horizontally (left = brighter), contrast drags vertically (up = more contrast).
- `createDragValueControl` gains an `invert` option that reverses the axis's default direction (x rightward, y upward), so direction stays separable from the sensitivity magnitude.
