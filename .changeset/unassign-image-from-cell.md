...
Assignments for cells pruned by a shrink are still kept, so a shrink/expand round trip restores the image that cell was showing (its view transform is reset, as on any assignment). The cell-assignment helpers scope by the grid's cell count and ignore cells outside it, and `activeImageId` on both framework contexts now goes through `getActiveCellImageId` — so tool constraints, Convert-to-Rect and the Delete shortcut cannot act on an image that no visible cell is showing.
