...
- **`obj.id` is reserved for annotation objects.** It's how `ViewerCell` discriminates annotation objects from companion-layer objects on the same canvas (the clear filter is `obj => obj.id`, registered via `initFabricModule`'s `FabricObject.customProperties`). Non-annotation Fabric objects (decoration lines, drawing previews, future overlay graphics) MUST NOT carry an `id`. Use `_readOnly: true` so the overlay keeps them inert (`selectable: false, evented: false`) in every mode; do not register `_readOnly` as a serialized custom property. **`FabricOverlay.applyModeToObject(obj, readOnly)` is the single authority** — `setMode` applies it to every object on the canvas, and `ViewerCell` applies it to each object it rebuilds, because objects are added after `setMode` has already run. Never set `selectable`/`evented` on an annotation object directly: doing so in the rebuild silently undid `paint` mode on the first state change.
...
The project is split into thirteen packages with clear dependency boundaries:
...
- **`@osdlabel/mask`** (`packages/mask/`) — Raster mask storage and codecs. **Zero dependencies** — it does not import `@osdlabel/annotation`. Contains: the `MaskBuffer` interface (`get` / `set` / optional `reserve` / `snapshot`) and its `BoundedDenseMaskBuffer` implementation (dense `Uint8Array` over the painted bbox, grown in chunks, capped by `maxPixels`), `MaskSnapshot` / `MaskRegion`, stroke rasterization (`stampCircle`, `strokeSegment`), the canonical codec (`CanonicalMaskData`, `encodeCanonical`, `decodeCanonical`), the `MaskCodec` / `MaskCodecRegistry` extension point, and the built-in COCO codecs (`cocoRleCodec`, `cocoRleUncompressedCodec`, `cocoBbox`, `cocoArea`, `isCocoInteropSafe`).
...
- **`@osdlabel/mask` has zero dependencies — including on `@osdlabel/annotation`.** It is the codec layer, deliberately unaware of annotations. This is why the mask wire format is declared twice (`MASK_RAW_FORMAT` / `MaskRawData` there, `CANONICAL_MASK_FORMAT` / `CanonicalMaskData` here) and why `packages/osdlabel/src/mask-wire-format.ts` exists: `osdlabel` is the first package to see both, so it asserts at compile time that they still agree. If you change one side, change the other — the typecheck will tell you.
- **Mask pixels never live in geometry.** `MaskGeometry` carries only a bounding box and a pixel count; the pixels are in `rawAnnotationData`. This _breaks_ the invariant every vector geometry holds — that geometry alone reconstructs the shape — so anything that rebuilds a shape from geometry takes `VectorGeometry` (`Exclude<Geometry, MaskGeometry>`) and masks are a compile error there, not a runtime throw. Use `createMaskAnnotation` / `buildMaskFabricObject` instead.
- **The annotator's mask state is canonical, never a downstream format.** Painting, rendering, and state all speak `CanonicalMaskData` (row-major RLE, LEB128, base64). COCO RLE and any other format are applied only at the `serialize(state, { maskCodec })` boundary and decoded back at `deserialize(doc, { maskCodecs })`. Do not let COCO's column-major ordering or its signed-delta string encoding leak inward.
...
- `packages/mask/` — `@osdlabel/mask` (mask buffers, stroke rasterization, canonical + COCO codecs)
...
5. **`@osdlabel/mask`** — Mask storage and codecs. Zero deps. `import { BoundedDenseMaskBuffer, stampCircle, strokeSegment, encodeCanonical, decodeCanonical, cocoRleCodec, createMaskCodecRegistry, type MaskBuffer, type MaskSnapshot, type MaskCodec } from '@osdlabel/mask'`
6. **`@osdlabel/validation`** — Validation schemas. `import { BaseAnnotationSchema, OsdAnnotationSchema, OsdFieldsSchema, GeometrySchema, ToolTypeSchema, FabricRawAnnotationDataSchema } from '@osdlabel/validation'`
7. **`@osdlabel/fabric-annotations`** — Fabric annotation tools & utilities. `import { initFabricModule, RectangleTool, type ToolOverlay, type FabricFields, type FabricRawAnnotationData } from '@osdlabel/fabric-annotations'`
8. **`@osdlabel/fabric-osd`** — OSD overlay bridge & decoration renderer. `import { FabricOverlay, computeViewportTransform, DecorationLayer } from '@osdlabel/fabric-osd'`
9. **`osdlabel`** — Framework-agnostic shared logic. Types, serialization, pure reducers, constraints, keyboard mapping, tool factory. Also re-exports the decoration API.
...
10. **`@osdlabel/solid`** — SolidJS UI. Re-exports everything from `osdlabel` plus SolidJS-specific state/hooks/components.
...
11. **`@osdlabel/react`** — React UI. Re-exports everything from `osdlabel` plus React-specific state/hooks/components.
