...
/** Reassigned per test; read through the accessor below. */
let mockConstraintStatus: ConstraintStatus;
 ...
 *
 * Every field the tests vary goes through the factory's typed `overrides` for
 * the same reason — overriding them in the `vi.mock` body below would put them
 * straight back into the unchecked region this exists to escape.
 */
...
// Mock constraints (used by useKeyboard internally via useConstraints)
const mockConstraints = {
  isToolEnabled: vi.fn().mockImplementation((type: string) => {
    const status = mockConstraintStatus as Record<string, { enabled: boolean }>;
    return status[type]?.enabled ?? true;
  }),
};
...
describe('useKeyboard', () => {
  ...
  describe('Tool Selection Shortcuts', () => {
    ...
    /**
     * Named for `isToolEnabled` historically, but that is not the gate:
     * `useKeyboard` destructures it as `_isToolEnabled` and never calls it, and
     * `mapKeyEventToActions` reads `constraintStatus()[tool].enabled`. Removing
     * the `constraintStatus` override below fails this test even with
     * `isToolEnabled` still returning false, which is what pins it down.
     */
